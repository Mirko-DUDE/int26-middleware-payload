...
## [2026-03-05] — Campo `absenceType` in `AbsenceLog`: opzionale e informativo

**Problema:**
Il campo `absenceType` in `AbsenceLog` era definito con `required: true` e senza
`admin.readOnly`, il che causava due problemi:
1. Un webhook Furious che non include il tipo assenza avrebbe fallito la creazione del record.
2. Il campo era modificabile dall'UI admin, incoerente con il pattern dei campi di log.

Inoltre, il webhook salvava il fallback `'unknown'` quando il tipo non era presente —
un valore artificiale che inquina i dati e può confondere il team HR.

**Decisione:**
- Rimosso `required: true` dal campo `absenceType` in `AbsenceLog.ts`
- Aggiunto `admin.readOnly: true` e `label: 'Tipo assenza'` per coerenza con gli altri campi informativi
- In `absenceWebhook.ts`: il salvataggio usa `body.absenceType ?? body.type ?? null` — nessun fallback artificiale
- Aggiunto `absenceType?: string` come campo esplicito nel tipo `FuriousAbsenceWebhookPayload`
- Nessuna modifica alla logica di approvazione — il campo è puramente informativo per il team HR

**Motivazione:**
Il tipo di assenza non partecipa alla logica di auto-approvazione (che si basa solo su `pseudo`).
Salvare `null` quando il campo è assente è più onesto di salvare `'unknown'`.
Il campo `text` libero (non select) è intenzionale: i valori vengono da Furious e una
modifica futura ai tipi su Furious non deve rompere la validazione del middleware.

**File aggiornati:**
- `src/collections/AbsenceLog.ts` — campo `absenceType` reso opzionale e readOnly
- `src/endpoints/absenceWebhook.ts` — tipo aggiornato, fallback rimosso
- `src/payload-types.ts` — rigenerato
...
## [2026-03-05] — AutoApprovalRules: debito tecnico noto — filtro per tipo assenza

**Problema:**
Emersa l'esigenza futura di poter auto-approvare solo certi tipi di assenza
per ciascun dipendente (es. solo smartworking, non ferie). Il worker attuale
approva qualsiasi tipo.

**Analisi API Furious:**
Non esiste un endpoint Furious per recuperare dinamicamente i tipi di assenza
configurati in /admin/config-absence/. Il campo `type` nel payload webhook
usa ID composti con suffisso per entità aziendale (es. `ferie_669692a9c68e4`,
`rol_682f4be40b738`), non stringhe semplici. I valori reali disponibili per
questa installazione si ricavano dai record storici in `AbsenceLog.absenceType`,
che viene già popolato da B1.

**Decisione attuale:**
Nessun filtro per tipo — tutti i tipi vengono auto-approvati se il pseudo
è in AutoApprovalRules. Accettabile per Release 1.

**Debito tecnico / TODO futuro:**
Quando arriverà l'esigenza del filtro per tipo:
1. Aggiungere campo `absenceTypes` (array, opzionale) a `AutoApprovalRules`.
   Se vuoto o assente = tutti i tipi. Se popolato = solo quei tipi.
2. I valori da inserire si leggono dai record `AbsenceLog.absenceType`
   accumulati in produzione — non da un endpoint Furious (non esiste).
3. Aggiornare la query in `processAbsence.ts` per filtrare anche su
   `absenceType` quando il campo è presente nella regola.
4. L'Opzione C (sincronizzazione da API Furious) non è percorribile —
   Furious non espone un endpoint per listare i tipi configurati.

**Limite ownership AutoApprovalRules:**
